...
************************************************************************************************************/	
	...
	function JSDragDropTree()
	{
		...
		/** edit by damon huang  */
		this.isUseHighLightItem=true;
		this.isUseFolderImage=false;
	}
		...
		*/
			...
			if(lis.length>0){
				...
				while(li){
					if(li.id){
						...
						var numericID = li.id;//li.id.replace(/[^0-9]/gi,'');
						if(numericID.length==0)numericID='A';
						var numericParentID = li.parentNode.parentNode.id;//li.parentNode.parentNode.id.replace(/[^0-9]/gi,'');
						if(numericID!='0'){
		...
		initTree : function()
		{
			...
			for(var no=0;no<menuItems.length;no++){
                ...
                /** edited by damon huang on 20120422   this.addEvent(aTag, 'click', this.showHideNode);*/
				if ( this.isUseHighLightItem ) this.addEvent(aTag, 'click', this.highlightItem);

				if(!noDrag)aTag.onmousedown = JSTreeObj.initDrag;
				if(!noChildren)aTag.onmousemove = JSTreeObj.moveDragableNodes;
				menuItems[no].insertBefore(img,aTag);
				//menuItems[no].id = 'dhtmlgoodies_treeNode' + nodeId;
				if ( this.isUseFolderImage ) {
