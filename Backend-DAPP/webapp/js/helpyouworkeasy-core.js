...
function WRequest(x,p,f,n,t){
   ...
   this.to=t;
	...
	function g_login(){
		var xurl="pages/organization/login.html";
	    openDialog("用户登录",xurl,true,380,280,null);
	}
	...
	function showHelpWin(w,h,xurl){
	   ...
	   openDialog("系统帮助",xurl,true,w,h,null);
	}
	...
	function setCurrentUser(user, rights){
	  g_currentUser=user;
	  gToken=user.token;
	  g_userRights=rights.split(",");
	  try{
	  	var ht="欢迎:&nbsp;"+g_currentUser.userName;
	...
	function g_changepass(){
	  if ( g_currentUser.userCode == 'Guest' ) {
	    showInfoWinWarn("不能修改匿名账户的密码");
	    return ;
	  }
	...
	function g_isPermit(rightId){
	  if ( g_userRights.length<1 ) return false;
	  return ( g_userRights.indexOf(rightId) != -1 );
	}
	/**
   ...
   function v_selectTag(v_currentTabIndex){
	...
	function openDialog(t,xurl,isModal,w,h,callback){
		...
		if ( !h ) h=200;
		if ( !w ) w=400;
		$( "#g_dialog" ).html('<iframe id="g_dialog_iframe" src="about:blank" width="100%" height="'+(h-60)+'px" frameborder="0"></iframe>');
		$( "#g_dialog" ).dialog({
			title: t,
			height: h,
			width: w,
            ...
            beforeClose: function(event, ui) {
            	try{
				  try{
					...
					document.getElementById("g_dialog_iframe").src = "about:blank";
				  }catch(e){}
 ...
 * @param callback
 */
function openMaximizationDialog(t,xurl,isModal,callback){
	isModal=true;
	var h=$(window).height();
	var w=$(window).width();

    var modleDivName='g_dialog';
    var modleFramename='g_dialog_iframe';
    // g_selectDialog2
    if ( xurl.indexOf('dialogLevel=2') != -1 ) {
       modleDivName='g_selectDialog2';
       modleFramename='g_selectDialog_iframe2';
    } else if ( xurl.indexOf('dialogLevel=1') != -1 ) {
       modleDivName='g_selectDialog';
       modleFramename='g_selectDialog_iframe';
    }

	$( "#"+modleDivName ).html('<iframe id="'+modleFramename+'" src="about:blank" width="100%" height="'+(h-60)+'px" frameborder="0"></iframe>');
	$( "#"+modleDivName ).dialog({
		title: t,
		height: h,
		width: w,
		...
		beforeClose: function(event, ui) {
			try{
				try{
					document.getElementById(modleFramename).contentWindow.doBeforeClose();
					document.getElementById(modleFramename).src = "about:blank";
				}catch(e){}
				if ( callback ) callback.apply();
			}catch(e){
			}
			$("#"+modleFramename).remove();
			return true;
		}
	});
	document.getElementById(modleFramename).src = xurl;
}
	...
	function openSelectDialog(t,xurl,w,h,callback){
		if ( !h ) h=200;
		if ( !w ) w=400;
		$( "#g_selectDialog" ).html('<iframe id="g_selectDialog_iframe" src="about:blank" width="100%" height="'+(h-45)+'px" frameborder="0"></iframe>');
	...
	function openSelectDialog2(t,xurl,w,h,callback){
		if ( !h ) h=200;
		if ( !w ) w=400;
		$( "#g_selectDialog2" ).html('<iframe id="g_selectDialog_iframe2" src="about:blank" width="100%" height="'+(h-45)+'px" frameborder="0"></iframe>');
	...
	*/
	...
	function doSendAjaxRequest(xurl,param,fnCallback,notShowError,to){
	    isRequesting = true;
	    if ( !notShowError ) notShowError = true; // 默认不提示异常信息
	    if ( xurl.indexOf('http://') == -1 && xurl.indexOf('https://') == -1 ) xurl = g_UrlBase+xurl;
		...
		$.ajax( {
			...
			"beforeSend": function(XMLHttpRequest) {
                XMLHttpRequest.setRequestHeader("token", gToken);
            },
			...
			"error": function (xhr, error, thrown) {
				try{
				  ...
				  if ( xurl.indexOf('getCurrentUserInfo') != -1 ) {
					  loginFail();
				  }
				}catch(e){
				}
	...
	function getDeptPath(deptId) {
		...
		while( node && node.parentId!=0 && n < 5){
	     n++;
	 ...
	 * @returns {String}
	 */
	function getCodeSelectHtmlSortByName(type,name,hasEmpty,initValue, onchangeFn, allName, ww){
		if ( initValue == undefined ) initValue = "";
		if ( !allName ) allName="&nbsp;&nbsp;";
		if ( !ww ) ww="120px";
		var ht="<select id='"+name+"' style='width:"+ww+"'>";
		...
		var codeList=getCodeListByType(type);
		...
		for(var i=0;i<codeList.length;i++ ){
			if ( type == codeList[i].type ) {
				ht+="<option value='"+codeList[i].code+"' ";
				if ( codeList[i].code == initValue ) ht+=" selected ";
				ht+=">"+codeList[i].name+"</option>";
			}
		}
		ht+="</select>";
		return ht;
	...
	*/
	 ...
	 * 返回某分类一类型的字典列表
	*/
	function getCodeListByTypeAndBelong(type,belong){
		var keys=g_dictionary.keySet();
		var code=null;
		var reList=[];
		for(var i=0;i<keys.length;i++ ){
			code = g_dictionary.get(keys[i]);
			if ( type == code.type && code.belongType == belong ) {
				reList[reList.length]=code;
			}
		}
		return reList;
