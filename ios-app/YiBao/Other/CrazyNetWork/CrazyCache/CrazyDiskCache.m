#import "CrazyDiskCache.h"
#import "CrazyCacheBackgroundTaskManager.h"

#if __IPHONE_OS_VERSION_MIN_REQUIRED >= __IPHONE_4_0
#import <UIKit/UIKit.h>
#endif

#define CrazyDiskCacheError(error) if (error) { NSLog(@"%@ (%d) ERROR: %@", \
                                    [[NSString stringWithUTF8String:__FILE__] lastPathComponent], \
                                    __LINE__, [error localizedDescription]); }

static id <CrazyCacheBackgroundTaskManager> CrazyCacheBackgroundTaskManager;

NSString * const CrazyDiskCachePrefix = @"com.tumblr.CrazyDiskCache";
NSString * const CrazyDiskCacheSharedName = @"CrazyDiskCacheShared";

@interface CrazyDiskCache ()
...
- (instancetype)initWithName:(NSString *)name rootPath:(NSString *)rootPath
{
    ...
    if (self = [super init]) {
        _name = [name copy];
        _queue = [CrazyDiskCache sharedQueue];
        ...
        NSString *pathComponent = [[NSString alloc] initWithFormat:@"%@.%@", CrazyDiskCachePrefix, _name];
        _cacheURL = [NSURL fileURLWithPathComponents:@[ rootPath, pathComponent ]];

        __weak CrazyDiskCache *weakSelf = self;

        dispatch_async(_queue, ^{
            CrazyDiskCache *strongSelf = weakSelf;
...
{
    return [[NSString alloc] initWithFormat:@"%@.%@.%p", CrazyDiskCachePrefix, _name, self];
}
...
+ (instancetype)sharedCache
{
    ...
    dispatch_once(&predicate, ^{
        cache = [[self alloc] initWithName:CrazyDiskCacheSharedName];
    });
...
+ (dispatch_queue_t)sharedQueue
{
    ...
    dispatch_once(&predicate, ^{
        queue = dispatch_queue_create([CrazyDiskCachePrefix UTF8String], DISPATCH_QUEUE_SERIAL);
    });
...
+ (dispatch_queue_t)sharedTrashQueue
{
    ...
    dispatch_once(&predicate, ^{
        NSString *queueName = [[NSString alloc] initWithFormat:@"%@.trash", CrazyDiskCachePrefix];
...
+ (NSURL *)sharedTrashURL
{
    ...
    dispatch_once(&predicate, ^{
        sharedTrashURL = [[[NSURL alloc] initFileURLWithPath:NSTemporaryDirectory()] URLByAppendingPathComponent:CrazyDiskCachePrefix isDirectory:YES];
...
+(BOOL)moveItemAtURLToTrash:(NSURL *)itemURL
{
    ...
    NSURL *uniqueTrashURL = [[CrazyDiskCache sharedTrashURL] URLByAppendingPathComponent:uniqueString];
...
+ (void)emptyTrash
{
    UIBackgroundTaskIdentifier taskID = [CrazyCacheBackgroundTaskManager beginBackgroundTask];
    ...
    dispatch_async([self sharedTrashQueue], ^{        
        ...
        [CrazyCacheBackgroundTaskManager endBackgroundTask:taskID];
    });
}
...
- (BOOL)removeFileAndExecuteBlocksForKey:(NSString *)key
{
    ...
    BOOL trashed = [CrazyDiskCache moveItemAtURLToTrash:fileURL];
    if (!trashed)
        return NO;
    
    [CrazyDiskCache emptyTrash];
...
- (void)objectForKey:(NSString *)key block:(CrazyDiskCacheObjectBlock)block
{
...
- (void)fileURLForKey:(NSString *)key block:(CrazyDiskCacheObjectBlock)block
{
    NSDate *now = [[NSDate alloc] init];
...
- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key block:(CrazyDiskCacheObjectBlock)block
{
    NSDate *now = [[NSDate alloc] init];
...
- (void)removeObjectForKey:(NSString *)key block:(CrazyDiskCacheObjectBlock)block
{
    if (!key)
        return;
...
- (void)trimToSize:(NSUInteger)trimByteCount block:(CrazyDiskCacheBlock)block
{
    if (trimByteCount == 0) {
...
- (void)trimToDate:(NSDate *)trimDate block:(CrazyDiskCacheBlock)block
{
    if (!trimDate)
        return;
...
- (void)trimToSizeByDate:(NSUInteger)trimByteCount block:(CrazyDiskCacheBlock)block
{
    if (trimByteCount == 0) {
        [self removeAllObjects:block];
        return;
...
- (void)removeAllObjects:(CrazyDiskCacheBlock)block
{
    UIBackgroundTaskIdentifier taskID = [CrazyCacheBackgroundTaskManager beginBackgroundTask];
    ...
    dispatch_async(_queue, ^{
        ...
        [CrazyDiskCache moveItemAtURLToTrash:strongSelf->_cacheURL];
        [CrazyDiskCache emptyTrash];
...
- (void)enumerateObjectsWithBlock:(CrazyDiskCacheObjectBlock)block completionBlock:(CrazyDiskCacheBlock)completionBlock
{
    if (!block)
        return;
...
- (id <NSCoding>)objectForKey:(NSString *)key
{
    ...
    [self objectForKey:key block:^(CrazyDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        objectForKey = object;
...
- (NSURL *)fileURLForKey:(NSString *)key
{
    ...
    [self fileURLForKey:key block:^(CrazyDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
...
- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key
{
    ...
    [self setObject:object forKey:key block:^(CrazyDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeObjectForKey:(NSString *)key
{
    ...
    [self removeObjectForKey:key block:^(CrazyDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToSize:(NSUInteger)byteCount
{
    ...
    [self trimToSize:byteCount block:^(CrazyDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToDate:(NSDate *)date
{
    ...
    [self trimToDate:date block:^(CrazyDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToSizeByDate:(NSUInteger)byteCount
{
    ...
    [self trimToSizeByDate:byteCount block:^(CrazyDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeAllObjects
{
    ...
    [self removeAllObjects:^(CrazyDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)enumerateObjectsWithBlock:(CrazyDiskCacheObjectBlock)block
{
    if (!block)
        return;
    ...
    [self enumerateObjectsWithBlock:block completionBlock:^(CrazyDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (CrazyDiskCacheObjectBlock)willAddObjectBlock
{
    __block CrazyDiskCacheObjectBlock block = nil;
...
- (void)setWillAddObjectBlock:(CrazyDiskCacheObjectBlock)block
{
    __weak CrazyDiskCache *weakSelf = self;
...
- (CrazyDiskCacheObjectBlock)willRemoveObjectBlock
{
    __block CrazyDiskCacheObjectBlock block = nil;
...
- (void)setWillRemoveObjectBlock:(CrazyDiskCacheObjectBlock)block
{
    __weak CrazyDiskCache *weakSelf = self;
...
- (CrazyDiskCacheBlock)willRemoveAllObjectsBlock
{
    __block CrazyDiskCacheBlock block = nil;
...
- (void)setWillRemoveAllObjectsBlock:(CrazyDiskCacheBlock)block
{
    __weak CrazyDiskCache *weakSelf = self;
...
- (CrazyDiskCacheObjectBlock)didAddObjectBlock
{
    __block CrazyDiskCacheObjectBlock block = nil;
...
- (void)setDidAddObjectBlock:(CrazyDiskCacheObjectBlock)block
{
    __weak CrazyDiskCache *weakSelf = self;
...
- (CrazyDiskCacheObjectBlock)didRemoveObjectBlock
{
    __block CrazyDiskCacheObjectBlock block = nil;
...
- (void)setDidRemoveObjectBlock:(CrazyDiskCacheObjectBlock)block
{
    __weak CrazyDiskCache *weakSelf = self;
...
- (CrazyDiskCacheBlock)didRemoveAllObjectsBlock
{
    __block CrazyDiskCacheBlock block = nil;
...
- (void)setDidRemoveAllObjectsBlock:(CrazyDiskCacheBlock)block
{
    __weak CrazyDiskCache *weakSelf = self;
...
+ (void)setBackgroundTaskManager:(id <CrazyCacheBackgroundTaskManager>)backgroundTaskManager {
    CrazyCacheBackgroundTaskManager = backgroundTaskManager;
}
