/**
 `CrazyDiskCache` is a thread safe key/value store backed by the file system. It accepts any object conforming
 ...
 The designated initializer for `CrazyDiskCache` is <initWithName:>. The <name> string is used to create a directory
 ...
 Because this cache is bound by disk I/O it can be much slower than <CrazyMemoryCache>, although values stored in
 `CrazyDiskCache` persist after application relaunch. Using <CrazyCache> is recommended over using `CrazyDiskCache`
...
@class CrazyDiskCache;
@protocol CrazyCacheBackgroundTaskManager;

typedef void (^CrazyDiskCacheBlock)(CrazyDiskCache *cache);
typedef void (^CrazyDiskCacheObjectBlock)(CrazyDiskCache *cache, NSString *key, id <NSCoding> object, NSURL *fileURL);

@interface CrazyDiskCache : NSObject
...
/**
 The URL of the directory used by this cache, usually `Library/Caches/com.tumblr.CrazyDiskCache.(name)`
...
/**
 ...
 For example:
    ...
    dispatch_sync([CrazyDiskCache sharedQueue], ^{
        NSLog(@"accurate, unchanging byte count: %d", [[CrazyDiskCache sharedCache] byteCount]);
    });
 */
 ...
 */
@property (copy) CrazyDiskCacheObjectBlock willAddObjectBlock;

/**
 A block to be executed just before an object is removed from the cache. The queue waits during execution.
 */
@property (copy) CrazyDiskCacheObjectBlock willRemoveObjectBlock;
 ...
 */
- (void)trimToDate:(NSDate *)date block:(CrazyDiskCacheBlock)block;
...
/**
 ...
 @param block A block to be executed serially after the cache has been trimmed, or nil.
 */
- (void)trimToSize:(NSUInteger)byteCount block:(CrazyDiskCacheBlock)block;
...
/**
 ...
 @param block A block to be executed serially after the cache has been trimmed, or nil.
 */
- (void)trimToSizeByDate:(NSUInteger)byteCount block:(CrazyDiskCacheBlock)block;
...
/**
 Set a global manager to be used for setting up/tearing down any background tasks needed by CrazyCache.
