...
NSString * const CrazyMemoryCachePrefix = @"com.tumblr.CrazyMemoryCache";

@interface CrazyMemoryCache ()
...
@implementation CrazyMemoryCache
...
{
    if (self = [super init]) {
        NSString *queueName = [[NSString alloc] initWithFormat:@"%@.%p", CrazyMemoryCachePrefix, self];
...
- (void)handleMemoryWarning
{
    ...
    __weak CrazyMemoryCache *weakSelf = self;
    
    dispatch_async(_queue, ^{
        CrazyMemoryCache *strongSelf = weakSelf;
        if (!strongSelf)
            return;
...
- (void)trimToAgeLimitRecursively
{
    ...
    dispatch_after(time, _queue, ^(void){
        ...
        __weak CrazyMemoryCache *weakSelf = strongSelf;
...
- (void)objectForKey:(NSString *)key block:(CrazyMemoryCacheObjectBlock)block
{
...
- (void)setObject:(id)object forKey:(NSString *)key block:(CrazyMemoryCacheObjectBlock)block
{
    [self setObject:object forKey:key withCost:0 block:block];
}

- (void)setObject:(id)object forKey:(NSString *)key withCost:(NSUInteger)cost block:(CrazyMemoryCacheObjectBlock)block
{
    NSDate *now = [[NSDate alloc] init];
...
- (void)removeObjectForKey:(NSString *)key block:(CrazyMemoryCacheObjectBlock)block
{
    if (!key)
        return;
...
- (void)trimToDate:(NSDate *)trimDate block:(CrazyMemoryCacheBlock)block
{
    if (!trimDate)
        return;
...
- (void)trimToCost:(NSUInteger)cost block:(CrazyMemoryCacheBlock)block
{
    __weak CrazyMemoryCache *weakSelf = self;
...
- (void)trimToCostByDate:(NSUInteger)cost block:(CrazyMemoryCacheBlock)block
{
    __weak CrazyMemoryCache *weakSelf = self;
...
- (void)removeAllObjects:(CrazyMemoryCacheBlock)block
{
    __weak CrazyMemoryCache *weakSelf = self;
...
- (void)enumerateObjectsWithBlock:(CrazyMemoryCacheObjectBlock)block completionBlock:(CrazyMemoryCacheBlock)completionBlock
{
    if (!block)
        return;
...
- (id)objectForKey:(NSString *)key
{
    ...
    [self objectForKey:key block:^(CrazyMemoryCache *cache, NSString *key, id object) {
        objectForKey = object;
...
- (void)setObject:(id)object forKey:(NSString *)key withCost:(NSUInteger)cost
{
    ...
    [self setObject:object forKey:key withCost:cost block:^(CrazyMemoryCache *cache, NSString *key, id object) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeObjectForKey:(NSString *)key
{
    ...
    [self removeObjectForKey:key block:^(CrazyMemoryCache *cache, NSString *key, id object) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToDate:(NSDate *)date
{
    ...
    [self trimToDate:date block:^(CrazyMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToCost:(NSUInteger)cost
{
    ...
    [self trimToCost:cost block:^(CrazyMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)trimToCostByDate:(NSUInteger)cost
{
    ...
    [self trimToCostByDate:cost block:^(CrazyMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)removeAllObjects
{
    ...
    [self removeAllObjects:^(CrazyMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)enumerateObjectsWithBlock:(CrazyMemoryCacheObjectBlock)block
{
    if (!block)
        return;
    ...
    [self enumerateObjectsWithBlock:block completionBlock:^(CrazyMemoryCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (CrazyMemoryCacheObjectBlock)willAddObjectBlock
{
    __block CrazyMemoryCacheObjectBlock block = nil;
...
- (void)setWillAddObjectBlock:(CrazyMemoryCacheObjectBlock)block
{
    __weak CrazyMemoryCache *weakSelf = self;
...
- (CrazyMemoryCacheObjectBlock)willRemoveObjectBlock
{
    __block CrazyMemoryCacheObjectBlock block = nil;
...
- (void)setWillRemoveObjectBlock:(CrazyMemoryCacheObjectBlock)block
{
    __weak CrazyMemoryCache *weakSelf = self;
...
- (CrazyMemoryCacheBlock)willRemoveAllObjectsBlock
{
    __block CrazyMemoryCacheBlock block = nil;
...
- (void)setWillRemoveAllObjectsBlock:(CrazyMemoryCacheBlock)block
{
    __weak CrazyMemoryCache *weakSelf = self;
...
- (CrazyMemoryCacheObjectBlock)didAddObjectBlock
{
    __block CrazyMemoryCacheObjectBlock block = nil;
...
- (void)setDidAddObjectBlock:(CrazyMemoryCacheObjectBlock)block
{
    __weak CrazyMemoryCache *weakSelf = self;
...
- (CrazyMemoryCacheObjectBlock)didRemoveObjectBlock
{
    __block CrazyMemoryCacheObjectBlock block = nil;
...
- (void)setDidRemoveObjectBlock:(CrazyMemoryCacheObjectBlock)block
{
    __weak CrazyMemoryCache *weakSelf = self;
...
- (CrazyMemoryCacheBlock)didRemoveAllObjectsBlock
{
    __block CrazyMemoryCacheBlock block = nil;
...
- (void)setDidRemoveAllObjectsBlock:(CrazyMemoryCacheBlock)block
{
    __weak CrazyMemoryCache *weakSelf = self;
...
- (CrazyMemoryCacheBlock)didReceiveMemoryWarningBlock
{
    __block CrazyMemoryCacheBlock block = nil;
...
- (void)setDidReceiveMemoryWarningBlock:(CrazyMemoryCacheBlock)block
{
    __weak CrazyMemoryCache *weakSelf = self;
...
- (CrazyMemoryCacheBlock)didEnterBackgroundBlock
{
    __block CrazyMemoryCacheBlock block = nil;
...
- (void)setDidEnterBackgroundBlock:(CrazyMemoryCacheBlock)block
{
    __weak CrazyMemoryCache *weakSelf = self;
