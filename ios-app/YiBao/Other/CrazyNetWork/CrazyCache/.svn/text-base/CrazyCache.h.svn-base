/**
 `CrazyCache` is a thread safe key/value store designed for persisting temporary objects that are expensive to
 reproduce, such as downloaded data or the results of slow processing. It is comprised of two self-similar
 stores, one in memory (<CrazyMemoryCache>) and one on disk (<CrazyDiskCache>).
 
 `CrazyCache` itself actually does very little; its main function is providing a front end for a common use case:
 ...
 repopulated in memory the next time they are accessed. `CrazyCache` also does the tedious work of creating a
 ...
 separately if necessary. See the docs for <CrazyMemoryCache> and <CrazyDiskCache> for more details.
 */

#import <Foundation/Foundation.h>

//#import <CrazyNetWork/CrazyDiskCache.h>
//#import <CrazyNetWork/CrazyMemoryCache.h>
...
typedef void (^CrazyCacheBlock)(CrazyCache *cache);
typedef void (^CrazyCacheObjectBlock)(CrazyCache *cache, NSString *key, id object);

@interface CrazyCache : NSObject
...
/**
 The underlying disk cache, see <CrazyDiskCache> for additional configuration and trimming options.
 */
@property (readonly) CrazyDiskCache *diskCache;

/**
 The underlying memory cache, see <CrazyMemoryCache> for additional configuration and trimming options.
 */
@property (readonly) CrazyMemoryCache *memoryCache;
