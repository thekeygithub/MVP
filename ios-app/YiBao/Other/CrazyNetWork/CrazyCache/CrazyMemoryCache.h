/**
 `CrazyMemoryCache` is a fast, thread safe key/value store similar to `NSCache`. On iOS it will clear itself
 ...
 Values will not persist after application relaunch or returning from the background. See <CrazyCache> for
 a memory cache backed by a disk cache.
 */

#import <Foundation/Foundation.h>

@class CrazyMemoryCache;

typedef void (^CrazyMemoryCacheBlock)(CrazyMemoryCache *cache);
typedef void (^CrazyMemoryCacheObjectBlock)(CrazyMemoryCache *cache, NSString *key, id object);

@interface CrazyMemoryCache : NSObject
 ...
 */
@property (copy) CrazyMemoryCacheObjectBlock willAddObjectBlock;
 ...
 */
@property (copy) CrazyMemoryCacheObjectBlock willRemoveObjectBlock;
 ...
 */
@property (copy) CrazyMemoryCacheBlock didReceiveMemoryWarningBlock;
 ...
 */
@property (copy) CrazyMemoryCacheBlock didEnterBackgroundBlock;
 ...
 */
- (void)trimToDate:(NSDate *)date block:(CrazyMemoryCacheBlock)block;
 ...
 */
- (void)handleMemoryWarning __deprecated_msg("This happens automatically in CrazyCache 2.1. There’s no longer a need to call it directly.");

/**
 Handle the application having been backgrounded.
 */
- (void)handleApplicationBackgrounding __deprecated_msg("This happens automatically in CrazyCache 2.1. There’s no longer a need to call it directly.");
