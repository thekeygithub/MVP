...
@interface BrushFacePayViewController ()<CaptureManagerDelegate>
{
    UILabel *alignLabel;
    int number;//
    int takePhotoNumber;
...
@implementation BrushFacePayViewController
...
-(void)makeUI
{
    ...
    //图片放置View
    imageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight - HEIGHT(60))];
    [backView addSubview:imageView];
    
    //    //button上传图片
    //    [self buttonWithTitle:@"上传图片" frame:CGRectMake(0, ScreenHeight - HEIGHT(60), ScreenWidth/2, HEIGHT(60)) action:@selector(didClickUpPhoto) AddView:backView];
    //
    //    //重拍图片按钮
    //    [self buttonWithTitle:@"重拍" frame:CGRectMake(ScreenWidth/2, ScreenHeight - HEIGHT(60), ScreenWidth/2, HEIGHT(60)) action:@selector(didClickPhotoAgain) AddView:backView];
    
//    UIImageView *backImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight)];
//    backImageView.image = [UIImage imageNamed:@"刷脸"];
...
-(void)didClickTakePhoto
{
    ...
    [myStillImageOutput captureStillImageAsynchronouslyFromConnection:myVideoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
        ...
        if (imageDataSampleBuffer) {
            ...
            //
            //            imageView.frame = CGRectMake(0, 0, ScreenWidth, ScreenHeight - HEIGHT(60));
            //            imageView.contentMode = UIViewContentModeScaleAspectFit;
            //            [self.view addSubview:backView];
            [self.faceDelegate sendFaceImage:myImage];
//            [self brushFaceData:myImage];
            //停止摄像
            [self.previewLayer.session stopRunning];
...
//#pragma mark - 刷脸支付
//- (void)brushFaceData:(UIImage *)image
//{
