/**
 * jQuery hollybeaconui 1.1
 * 来自jQuery easyui 1.4.1
 */
var loadingPage = {
	loading: function(options) {
		...
		if (options.open == true) {
			var loading = '<div class="loadingPage"><div class="loading-mask" style="display:block;"></div><div class="loading-mask-msg" style="display:block;"><div class="loading-mask-msgbg"></div><div class="loading-mask-msgtext"></div></div></div>'
...
(function($) {
	function _a0(_a1) {
		...
		function _a8(_aa) {
			...
			a.wrap("<td></td>");
			a.linkbutton({ iconCls: btn.iconCls }).unbind(".pagination").bind("click.pagination", function() {
	...
	$.fn.pagination.defaults = {
		...
		displayMsg: " {from} 到 {to}  共{total} 条",
		nav: {
			first: {
			...
			refresh: {
				...
				handler: function() {
					...
					if (_d3.onBeforeRefresh.call(this, 1, _d3.pageSize) != false) {
						$(this).pagination("select", 1);
						_d3.onRefresh.call(this, 1, _d3.pageSize);
					}
				}
			}
		}
...
(function($) {
	...
	function _186(_187, _188) {
		...
		_18a.width(_189);
		_18a.height(document.compatMode == "CSS1Compat" ? (22 - (_18a.outerHeight() - _18a.height())) : 22);
	...
	$.fn.tree.defaults = {
		...
		loader: function(_1bb, _1bc, _1bd) {
			...
			$.ajax({
				type: opts.method,
				url: doUrl(opts.method, opts.url),
				data: _1bb,
				dataType: "json",
...
(function($) {
	...
	function _20f(_210) {
		...
		function _213() {
			...
			if (opts.title && !opts.noheader) {
				...
				if (opts.collapsible) {
					$("<a class=\"panel-tool-collapse\" href=\"javascript:void(0)\" title=\"收起\"></a>").appendTo(tool).bind("click", function() {
				...
				if (opts.minimizable) {
					$("<a class=\"panel-tool-min\" href=\"javascript:void(0)\" title=\"最小化\"></a>").appendTo(tool).bind("click", function() {
						_23b(_210);
						return false;
					});
				}
				if (opts.maximizable) {
					$("<a class=\"panel-tool-max\" href=\"javascript:void(0)\" title=\"最大化\"></a>").appendTo(tool).bind("click", function() {
				...
				if (opts.closable) {
					$("<a class=\"panel-tool-close\" href=\"javascript:void(0)\" title=\"关闭\">&times;</a>").appendTo(tool).bind("click", function() {
						_229(_210);
						return false;
					});
				}
	...
	function _228(_232, _233) {
		...
		tool.addClass("panel-tool-expand").attr('title', '展开');
		if (_233 == true) {
	...
	function _235(_236, _237) {
		...
		tool.removeClass("panel-tool-expand").attr('title', '收起');
		if (_237 == true) {
	...
	function _227(_239) {
		...
		tool.addClass("panel-tool-restore").attr('title', '还原大小');
	...
	function _23e(_23f) {
		...
		_240.show();
		tool.removeClass("panel-tool-restore").attr('title', '最大化');
...
(function($) {
	...
	function _26d(_26e) {
		...
		var win = $(_26e).panel($.extend({}, _26f.options, {
			...
			onOpen: function() {
				...
				_25f(_26e);
				if (_26f.mask) {
					_26f.mask.css({ width: (opts.inline ? _26f.mask.parent().width() : _273().width), height: (opts.inline ? _26f.mask.parent().height() : _273().height), display: "block", zIndex: $.fn.window.defaults.zIndex++ });
				}
				if (_26f.shadow) {
	...
	function _273() {
		return { width: Math.max($(document.body)._outerWidth(), $(window).width()), height: Math.max($(document.body)._outerHeight(), $(window).height()) };
	};
...
(function($) {
	...
	function _294(_295) {
		...
		win.window(opts);
		win.window("window").css(opts.style).addClass('messager');
		win.window("open");
		return win;
	};
	...
	function _296(_297, _298, _299) {
		...
		if (_299) {
			...
			for (var _29a in _299) {
				$("<a></a>").attr("href", "javascript:void(0)").addClass(_29a == $.messager.defaults.ok ? 'l-btn-primary' : '').text(_29a).bind("click", eval(_299[_29a])).appendTo(tb).linkbutton();
			}
		}
		win.window({
			title: _297,
	...
	$.messager = {
		...
		confirm: function(_29f, msg, fn, ischangeIcon) {
			var _2a0 = '';
			if(ischangeIcon == null){
				_2a0 = "<div class=\"messager-icon messager-question\"></div>" + "<div>" + msg + "</div>" + "<div style=\"clear:both;\"/>";
			}else{
				_2a0 = "<div class=\"messager-icon messager-info\"></div>" + "<div>" + msg + "</div>" + "<div style=\"clear:both;\"/>";
			}
			var _2a1 = {};
...
(function($) {
	...
	function _332(_333, _334) {
		...
		if (_334.type == "all" || _334.type == "header") {
			...
			if (_title.indexOf('<span') != -1) {
				var attrTitle = _title.substr(0, _title.indexOf('<span'));
			} else {
				var attrTitle = _title;
			}
			var _337 = tab.find("span.tabs-title");
			var _338 = tab.find("span.tabs-icon");
			tab.find("a.tabs-inner").attr("title",attrTitle);
			...
			if (opts.tools) {
				...
				var pr = _339.children().length * 16;
				if (opts.closable) {
					pr += 8;
				} else {
					pr -= 3;
...
(function($) {
	...
	$.fn.validatebox.defaults = {
		...
		tipOptions: {
			...
			zIndex: "",
			onShow: function() {
				$(this).tooltip("tip").css({ color: "#fff", borderColor: "#e60000", backgroundColor: "#e60000" });
			},
			onHide: function() {
				$(this).tooltip("destroy");
			}
...
(function($) {
	...
	function _463(_464, _465) {
		...
		_46a.css({ width: opts.iconWidth + "px", height: tb.height() + "px", lineHeight: tb.height() + "px" });
	...
	$.fn.textbox.methods = {
		...
		setText: function(jq, _488) {
			return jq.each(function() {
				...
				_488 = _488 == undefined ? "" : String(_488);
	...
	$.fn.textbox.parseOptions = function(_494) {
		var t = $(_494);
		return $.extend({}, $.fn.validatebox.parseOptions(_494), $.parser.parseOptions(_494, ["width","height","separator",{panelWidth:"number",editable:"boolean",hasDownArrow:"boolean",delay:"number"}]), { value: (t.val() || undefined), type: (t.attr("type") ? t.attr("type") : undefined), disabled: (t.attr("disabled") ? true : undefined), readonly: (t.attr("readonly") ? true : undefined) });
	};
	...
	function _645(_646, _647) {
		...
		if (tr.length) {
			...
			if (top < 0) {
				...
				if (top + tr._outerHeight() > _64a.height() - 30) {
					_64a.scrollTop(_64a.scrollTop() + top + tr._outerHeight() - _64a.height() + 30);
				}
			}
		}
	};
			...
			return {
				...
				resize: function(_6dd, _6de) {
					if (isA(_6dd, name)) {
						...
						$(_6dd)._outerWidth(_6de)._outerHeight(30);
					}
				}
			};
		};
	...
	var _6df = $.extend({}, _6d1(["text", "textbox", "numberbox", "numberspinner", "combobox", "combotree", "combogrid", "datebox", "datetimebox", "timespinner", "datetimespinner"]), {
		...
		validatebox: {
			...
			resize: function(_6f6, _6f7) {
				$(_6f6)._outerWidth(_6f7)._outerHeight(30);
			}
		}
	});
	$.fn.datagrid.methods = {
...
(function($) {
	...
	$.fn.treegrid.defaults = $.extend({}, $.fn.datagrid.defaults, {
		...
		singleSelect: true,
		view: _88b,
		rowEvents: $.extend({}, $.fn.datagrid.defaults.rowEvents, { mouseover: _81c(true), mouseout: _81c(false), click: _81e }),
	...
	$.fn.combobox.defaults = $.extend({}, $.fn.combo.defaults, {
		...
		finder: {
			...
			getRow: function(_964, p) {
				...
				var _966 = (p instanceof $) ? p.attr("id").substr(_965.itemIdPrefix.length + 1) : _91d(_964, p);
...
(function($) {
	function _9af(_9b0) {
		...
		function _9b3(_9ba) {
			...
			var _9bc = $("<div class=\"datebox-button\"><table cellspacing=\"0\" cellpadding=\"0\"><tr></tr></table></div>").appendTo(_9bb);
			var tr = _9bc.find("tr");
			for (var i = 0; i < opts.buttons.length; i++) {
				var td = $("<td></td>").appendTo(tr);
	...
	$.fn.datebox.defaults = $.extend({}, $.fn.combo.defaults, {
		...
		buttons: [{
			...
			text: function(_9d7) {
				return $(_9d7).datebox("options").clearText;
			},
...
(function($) {
	...
	$.fn.datetimebox.defaults = $.extend({}, $.fn.datebox.defaults, {
		...
		buttons: [{
			...
			text: function(_9f8) {
				return $(_9f8).datetimebox("options").clearText;
			},
			handler: function(_9f9) {
				$(_9f9).datetimebox('clear')
...
function doUrl(method, url) {
	if (method && method.toUpperCase() == "GET") {
		return url + ((url.indexOf("?") >= 0 ? "&" : "?") + "curtime=" + new Date().getTime());
	} else {
		return url;
	}
}
...
$.extend($.fn.textbox.methods, {
	// textbox清除按钮
	addClearBtn: function(jq, config) {
		return jq.each(function() {
			var t = $(this);
			var opts = t.textbox('options');
	...
	// combo清除按钮
	comboClearBtn: function(jq, iconCls) {
		return jq.each(function() {
			var t = $(this);
			var opts = t.textbox('options');
			opts.icons = opts.icons || [];
			...
			t.combo('options').onChange = function() {
				var icon = t.textbox('getIcon', 0);
