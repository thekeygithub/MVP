 ...
 * 
 */
public class BussinessRunner2 implements Runnable {

	private static final Logger logger = Logger
			.getLogger(BussinessRunner2.class);
	...
	public BussinessRunner2(Map<String, Object> inParam,
	...
	public void run() {
		...
		try {
			...
			String ipfsHash2 = inParam.get("ipfsHash") == null ? "" : inParam
					.get("ipfsHash").toString();
			if (null == ipfsHash2) {
				...
				Map<String, Object> userInfoMap = IpfsUtil.getUserInformation(
						getUserInformationUrl, ipfsHash2, "1");
				this.inParam.put("userInfoMap", userInfoMap);// IPFS用户基本信息
				logger.info(validKey + " IPFS上取到的用户信息：" + userInfoMap);
				Map<String, Object> groupListMap = IpfsUtil.getUserInformation(
						getUserInformationUrl, ipfsHash2, "2");// 处方信息
				...
				if (null == userInfoMap) {
					...
					String type = inParam.get("type") == null ? "" : inParam
							.get("type").toString();
				...
				try {
					// 获取分账单
					pointsBills = ThirdInterfaceUtil.getPointsBills(inParam,
							env, interfaceHisList);
					logger.info(validKey + " 分账单：" + pointsBills);
				} catch (Exception e2) {
					logger.error(validKey + " 获取分账单 error " + e2.getMessage(),
							e2);
				}
			...
			TblTransactionInfo tblTransactionInfo = new TblTransactionInfo(
					txid, "验证合约", inParam.get("senderName") == null ? ""
							: inParam.get("senderName").toString(), "", "",
					"0", DateUtil.getNow(), validKey,
					inParam.get("serialNo") == null ? "" : inParam.get(
							"serialNo").toString(), "验证反馈");
			...
			// 调用医院通知完成接口
			String hospitalUrl = env.getProperty("hospital2.url");
