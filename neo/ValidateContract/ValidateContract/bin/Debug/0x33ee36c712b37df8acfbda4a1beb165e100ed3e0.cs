...
namespace ValidateContract
{
    ...
    {
        public static readonly byte[] Owner = "ARQW6J1RcF9zUPU6CvDZJdXuwvRSW4RCsy".ToScriptHash();
        ...
        public static object Main(string operation, object[] args)
        {
            ...
            {
                if (operation == "saveValidateInfo") return saveValidateInfo((string)args[0], (string)args[1]);
                if (operation == "savePayInfo") return savePayInfo((string)args[0], (string)args[1]);
         ...
         * 存储信息，返回获取数据的key
         * **/
        public static string saveValidateInfo(string key, string value)
        {
            //var txid = (ExecutionEngine.ScriptContainer as Transaction).Hash;
            //Storage.Put(Storage.CurrentContext, Helper.AsString(txid), Helper.AsString(txid));
            Storage.Put(Storage.CurrentContext, key, value);
            return key;
        }
        public static string savePayInfo(string key, string value)
        {
             Storage.Put(Storage.CurrentContext, key, value);
            return key;
        }
