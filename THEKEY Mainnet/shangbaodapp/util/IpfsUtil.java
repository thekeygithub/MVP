...
import com.xczg.blockchain.yibaodapp.bean.VerifyInfo;
...
public class IpfsUtil {
	...
	public static Map<String, Object> getUserInformation(String url,
		...
		try {
			...
			String resJson = HttpJsonBlockListClient.requestNeo(url,req.toJSONString());
	 ...
	 * @return
	 */
	public static String putVerifyInfo(String url,VerifyInfo verifyInfo) {
		Map<String, Object> req = new HashMap<String, Object>();
		req.put("DateTime", verifyInfo.getDateTime());
		req.put("HealthId", verifyInfo.getHealthId());
		req.put("HospId", verifyInfo.getHospId());
		req.put("IdNumber", verifyInfo.getIdNumber());
		req.put("InsuName", verifyInfo.getInsuName());
		req.put("Image", verifyInfo.getImage());
		req.put("Scale", verifyInfo.getScale());
		req.put("TreatmentId", verifyInfo.getTreatmentId());
		req.put("Type", verifyInfo.getType());
		req.put("UserName", verifyInfo.getUserName());
		String reqJson = JSONObject.toJSONString(req).toString();
		log.info("Ipfs putDmiResult 请求：" + reqJson);
		String resJson = HttpJsonBlockListClient.requestNeo(url, reqJson);
		log.info("Ipfs putDmiResult 响应：" + resJson);
		Map respMap = (Map) JSONObject.parseObject(resJson);
		String newHash = respMap.get("newHash") == null ? "" : respMap.get("newHash").toString();
		return newHash;
	}
	 ...
	 * @return
	 */
	public static String putInfoToIPFS(String url,String message) {
		log.info("Ipfs put 请求：" + message);
		try {
			
			Map<String, Object> req = new HashMap<String, Object>();
			req.put("hashId", "5699844984");
			Map<String, Object> dmiResult = new HashMap<String, Object>();
			req.put("dmiResult", dmiResult);
			Map<String, Object> resultInfo = new HashMap<String, Object>();
			...
			//String resJson =HttpUtil.doPostJson(url, reqJson);
			String resJson = HttpJsonBlockListClient.requestNeo(url, reqJson);
			log.info("Ipfs put 响应：" + resJson);
			Map respMap = (Map) JSONObject.parseObject(resJson);
			String newHash = respMap.get("newHash") == null ? "" : respMap.get("newHash").toString();
			return newHash;
		}catch(Exception e) {
	   ...
	   public static void main(String[] args) {
	    	//String hash=putResultInfo("http://222.128.14.106:2699/IPFS/api/putDmiResult","{\"msg\":\"ok\"}");
	    	//System.out.println("hash="+hash);
	    	String a = putInfoToIPFS("http://222.128.14.106:2699/IPFS/api/putDmiResult","{\"msg\":\"hello\"}");
	    	System.out.println("返回结果："+a);
	    }
	public static String putResultInfo(String url,String result) {
		String reqJson = JSONObject.toJSONString(result).toString();
		log.info("Ipfs putDmiResult 请求：" + reqJson);
		String resJson = HttpJsonBlockListClient.requestNeo(url, reqJson);
		log.info("Ipfs putDmiResult 响应：" + resJson);
		Map respMap = (Map) JSONObject.parseObject(resJson);
	...
	public static String putRelifyInfo(String url,String relifyInfo) {
		String reqJson = JSONObject.toJSONString(relifyInfo).toString();
		log.info("Ipfs putDmiResult 请求：" + reqJson);
		String resJson = HttpJsonBlockListClient.requestNeo(url, reqJson);
		log.info("Ipfs putDmiResult 响应：" + resJson);
		Map respMap = (Map) JSONObject.parseObject(resJson);
    ...
    public static void main1(String[] args) {
    	//String hash=putResultInfo("http://222.128.14.106:2699/IPFS/api/putDmiResult","{\"msg\":\"ok\"}");
    	//System.out.println("hash="+hash);
    	String hash=putInfoToIPFS("http://222.128.14.106:2699/IPFS/api/putUserInformation","{\"msg\":\"hello\"}");
